...
template <typename T>
class atomic {
    static_assert(ArchAtomics::is_implemented<T>());
    T			_v;
public:
			atomic (void) = default;
    ...
    inline bool		is_lock_free (void) const
			    { return ArchAtomics::is_lock_free<T>(); }
...
public:
    ...
    void		clear (memory_order order = memory_order_seq_cst)
			    { (void)order; ArchAtomics::store (_v, false); }
    bool		test_and_set (memory_order order = memory_order_seq_cst)
			    { (void)order; return ArchAtomics::exchange(_v, true); }
};
