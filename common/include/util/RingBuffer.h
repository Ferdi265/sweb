...
void RingBuffer<T>::put ( T c )
{
  ...
  ArchThreads::atomic_exchange<size_t> ( write_pos_, ( old_write_pos + 1 ) % buffer_size_ );
}

template <class T>
void RingBuffer<T>::clear()
{
  ArchThreads::atomic_exchange<size_t> ( write_pos_,1 );
  // assumed that there is only one reader who can't have called clear and get at the same time.
  // here get would return garbage.
  ArchThreads::atomic_exchange<size_t> ( read_pos_,0 );
}
...
bool RingBuffer<T>::get ( T &c )
{
  ...
  ArchThreads::atomic_exchange<size_t> ( read_pos_,new_read_pos );
  return true;
}
